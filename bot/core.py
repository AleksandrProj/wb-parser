...
from bot.main_func import insert_row
from bot.export_excel import ExportExcel
from bot.settings import FILES, USER_AGENT
...
class WildberriesBot:
    def __init__(self):
        ...
        self.num_page = 0
        self.query = input('Введите название товара или категории: ')
        self.quantity_page = int(input('Сколько страниц парсить (0 - парсить все): '))
    ...
    def parse_page(self, res):
        ...
        file = os.path.join(FILES['list_goods'], 'list_good_' + res['query'] + '.xlsx')
        self.num_page += 1
        ...
        if self.num_page <= self.quantity_page or self.quantity_page == 0:
            ...
            for card_good in table_goods:
                ...
                info_card_photo_good = self.browser.find_element_by_class_name('card-left').find_element_by_class_name(
                    'MagicZoomFullSizeImage').get_attribute('src')
